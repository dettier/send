...
SendStream.prototype.send = function(path, stat){
  ...
  var statSize = Buffer.byteLength(options.prefix) + stat.size + Buffer.byteLength(options.postfix);
  var len = statSize;
  var res = this.res;
  var req = this.req;
...
SendStream.prototype.stream = function(path, options){
  ...
  if (options.prefix.length > 0) {
    ...
    stream.on('data', function () {
      if (firstChunk) {
        res.write(options.prefix);
        firstChunk = false;
      }
    });
  }

  if (options.postfix.length > 0) {
    stream.on('end', function () {
      res.write(options.postfix);
    });
  }
